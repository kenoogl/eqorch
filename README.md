...
`KnowledgeIndex` は PostgreSQL 正本から再構築可能な補助インデックスとして扱う。`ArtifactStore` は大容量ログ、外部生成物、中間ファイルを保存する optional な Object Storage 補助層として扱う。どちらも正本ではなく、欠損時も PostgreSQL 正本の再現性を壊さない。
...
PostgreSQL 正本運用 readiness の判断事項は [ADR 001](/Users/Daily/Development/EqOrch/.kiro/steering/adrs/001-postgresql-readiness.md) に整理している。
