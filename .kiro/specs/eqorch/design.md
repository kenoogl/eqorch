...
現行実装フェーズでは `KnowledgeIndex` を補助層の実装対象に含め、`ArtifactStore` は後続開発の拡張コンポーネントとして扱う。
...
- `KnowledgeIndex` は現行実装フェーズで導入する optional な Vector DB 補助インデックスを指す
- `ArtifactStore` は後続開発フェーズで導入する optional な Object Storage 補助ストアを指す
...
- 現行実装フェーズで導入し、補助層失敗時も正本再現性を壊さないことを優先検証する
...
- 後続開発フェーズで導入する拡張コンポーネントとして扱い、現行フェーズでは契約と failure semantics のみを維持する
...
### PostgreSQL 本運用 readiness

- `WorkflowStore` と `TraceStore` について、スキーマ初期化方法を固定する
- schema migration 導入有無を判断し、その運用方針を明示する
- 接続プール要否を判断し、単一接続実装のまま運用するかを明示する
- 障害時に出すログ、通知イベント、終了コードを整理し、正本失敗と補助層失敗で区別する
- これらの判断項目は README、運用ドキュメント、または ADR から参照可能にする
...
- Vector DB / Object Storage 失敗:
  ...
  - 現行実装フェーズでは Vector DB 側を実装対象とし、Object Storage 側は後続開発で同じ failure semantics を適用する
...
- PostgreSQL 実 DB 接続を用いた継続検証経路で、WorkflowStore 正本保存、TraceStore 保存、ReplayLoader 復元を確認する
...
- `KnowledgeIndex` の optional 反映成功と、失敗時に正本再現性が壊れないこと
- `ArtifactStore` は後続開発対象として、現行フェーズでは契約と failure semantics の整合を確認対象に留める
...
- 単体、結合、E2E、非機能検証はカテゴリ別に独立した標準コマンドまたはスクリプトで実行できるようにする
- PostgreSQL 実 DB を用いる検証では、接続先設定と起動条件を明示した標準実行方法を提供する
- 日常開発では unit、マージ前には integration、E2E、performance を実行する

### 運用ドキュメント

- README または運用ドキュメントは少なくともセットアップ、CLI 起動、batch 起動、resume、PostgreSQL 実 DB 検証手順、必要環境変数を含む
- README または運用ドキュメントは `KnowledgeIndex` が現フェーズ対象、`ArtifactStore` が後続開発対象であることを明記する
- 運用ドキュメントは PostgreSQL 正本運用時の注意事項と、正本失敗 / 補助層失敗の failure semantics を含む
