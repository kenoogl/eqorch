...
現行実装フェーズでは `KnowledgeIndex` と `ArtifactStore` を補助層の実装対象に含める。
...
- 現行実装フェーズでは local backend を含む Object Storage 補助ストアとして導入し、補助層失敗時も正本再現性を壊さないことを優先する
...
- Vector DB / Object Storage 失敗:
  ...
  - 現行実装フェーズでは Vector DB 側と Object Storage 側の両方を optional 補助層として扱い、同じ failure semantics を適用する
...
- `ArtifactStore` へのオブジェクト保存成功系と補助層失敗系を結合テストで確認する
...
- README または運用ドキュメントは `KnowledgeIndex` と `ArtifactStore` が optional な補助層であることを明記する
