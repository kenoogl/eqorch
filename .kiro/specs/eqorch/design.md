...
| 8 | `WorkingMemory`, `PersistentMemoryStore`, `TraceStore`, `ReplayLoader`, `KnowledgeIndex`, `ArtifactStore` | 2 層メモリ、正本永続化、補助ストア |
...
- `PersistentMemoryStore` は退避済みデータと再開用スナップショットを保持する永続層を指す
...
- PostgreSQL を既定実装として State、Candidate、Evaluation、Policy 改訂履歴、モード遷移履歴を永続化
- `TraceStore`、`KnowledgeIndex`、`ArtifactStore` への書込み調停
- 起動時 / 再起動時のロード

設計判断:

- PostgreSQL を再現と再開の唯一の正本とする
- `TraceStore` は PostgreSQL を正本としつつ `JSON Lines` へエクスポート可能にする
- `KnowledgeIndex` は optional な Vector DB とし、正本にはしない
- `ArtifactStore` は optional な Object Storage とし、大容量中間生成物の退避先に使う
