...
class OrchestrationLoop:
    ...
    def run_cycle(self, state: State, *, issued_at: str | None = None) -> LoopCycleResult:
        ...
        next_state.last_errors = {}

        context = self._context_assembler.assemble(next_state)
        ...
        if len(actions) > next_state.policy_context.max_parallel_actions:
        ...
        if decision.coordinated_error is not None and decision.coordinated_error.should_record_last_error:
            next_state.last_errors[actions[0].action_id] = decision.coordinated_error.error
        ...
        for action in actions:
            before_state = deepcopy(next_state)
            record = self._dispatcher.dispatch([action], next_state)[0]
            ...
            if record.result.status == "success":
                ...
                if coordinated.should_record_last_error:
                    next_state.last_errors[record.action.action_id] = coordinated.error
                plan = self._trace_recorder.record(
                    action=record.action,
                    result=record.result,
                    previous_state=before_state,
        ...
        self._persistent_store.commit(
            PersistenceCommit(
                state_snapshot=deepcopy(next_state),
                state_summaries={"step": next_state.step, "mode": next_state.current_mode},
